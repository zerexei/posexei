...
Located in `terraform/`, organized by modules:
...
Manifests are managed in `k8s/`:
- `k8s/app/`: Core application manifests (PHP-FPM, Traefik, Workers).
- `k8s/observability/`: Helm value files and OpenTelemetry Collector configs.
