...
- **Dashboard**: [http://mailpit.localhost](http://mailpit.localhost) -->
...
Located in the `/infra/terraform/` directory.
...
Managed via `kubectl` and the manifests in `/infra/k8s/`.
...
Automatic deployments are triggered by pushes to the `production` branch.
